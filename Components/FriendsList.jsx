...
export default class FriendsList extends Component {
  ...
  componentDidMount() {
    ...
    firebase
      .database()
      .ref("/users/" + user.uid + "/data/friends")
      .on("value", (snapshot) => {
            ...
            firebase
              ...
              .once("value", (snapshot) => {
                if (snapshot.val() != null) {
                  name2 = snapshot
                    .val()
                    .username.slice(0, snapshot.val().username.indexOf("#"));
                  if (name != name2) {
                    //friend updated name
                    name = name2;
                    updates[
                      "/users/" + user.uid + "/data/friends/" + (index + 1)
      ...
      await Promise.all(promises).then(() => {
        ...
        if (removeFriend.length > 0) {
          updates["/users/" + user.uid + "/data/friends"] =
            this.props.userData.friends.filter(
              (friend, index) => !removeFriend.includes(index)
            );
        }
    ...
    firebase
      ...
      .once("value", (snapshot) => {
        ...
        if (foundUserData.friends.includes(this.props.userData.username)) {
          ...
          -1 !=
          foundUserRequest.friend_request.findIndex(
            (obj) => obj.username == this.props.userData.username
          )
        ) {
  ...
  SendFriendRequest() {
    ...
    this.state.foundUserRequest.friend_request.push({
      username: this.props.userData.username,
      photoURL: this.props.userData.photoURL,
    });
    var updates = {};

    const friendID = this.state.foundUserData.username.slice(
      this.state.foundUserData.username.indexOf("#") + 1
    );
  ...
  async RespondFriendRequest(friendName, accept) {
    ...
    newFriendRequest.splice(
      this.props.userRequest.friend_request.indexOf(friendName),
      1
    );
    ...
    if (accept) {
      ...
      var friend_confirmed = await firebase
        .database()
        .ref("/users/" + friendID + "/request/friend_confirmed")
        .once("value")
        .then((snapshot) => {
          return snapshot.val();
        });
      if (friend_confirmed !== null) {
        //Friend exists
        ...
        updates["/users/" + friendID + "/request/friend_confirmed"] =
          friend_confirmed;
      } else {
        Alert.alert(
          "User: " + friendName,
          "This user no longer exists. The user has deleted their account."
        );
      }
    }
  ...
  DisplayFoundUser() {
    return (
      ...
      >
        <View
          style={[
            styles.friendRequestBubble,
            { backgroundColor: "white", marginTop: 50 },
          ]}
        >
          <View style={{ flexDirection: "row", justifyContent: "center" }}>
            <Image
              source={{ uri: this.state.foundUserData.photoURL }}
              style={styles.avatarImage}
            />
          </View>

          <Text style={[styles.textStyleDark, { fontSize: 20 }]}>
            {this.state.foundUserData.username.slice(
              0,
              this.state.foundUserData.username.indexOf("#")
            )}
          </Text>
          ...
          <TouchableOpacity
            ...
            style={[
              ...
              this.state.userAbleAdd
                ? { backgroundColor: "#2e8fff" }
                : styles.disabled,
            ]}
            onPress={() => {
              this.SendFriendRequest();
              this.setState({ foundModalVisible: false });
            }}
          >
        ...
        >
          <View style={styles.modalView}>
            {friendRequests.length == 0 ? (
              <Text style={[styles.titleButtonStyle, { color: "black" }]}>
                No Friend Requests
              </Text>
            ) : (
              <FlatList
                ...
                renderItem={({ item }) => {
                  return (
                    <View style={styles.friendRequestBubble}>
                      ...
                      <Text style={[styles.textStyleDark, { fontSize: 20 }]}>
                        {" "}
                        {item.username.slice(0, item.username.indexOf("#"))}
                      </Text>
                      ...
                      >
                        <TouchableOpacity
                          ...
                          onPress={() => {
                            this.RespondFriendRequest(item.username, true);
                          }}
                        >
                        ...
                        <TouchableOpacity
                          ...
                          onPress={() => {
                            this.RespondFriendRequest(item.username, false);
                          }}
                        >
  ...
  async RemoveFriend(username) {
    ...
    updates["/users/" + friendID + "/request/friend_delete"] = [
        ...
        <SafeAreaView style={{ flex: 1, backgroundColor: "#1B2430" }}>
          ...
          >
            ...
            <TouchableOpacity
              ...
              onPress={() =>
                this.setState({
                  mangageFriendsVisible: !this.state.mangageFriendsVisible,
                })
              }
            >
              ...
              <View style={{ width: "100%" }}>
                ...
                >
                  <Text style={styles.textStyle}>
                    Friend Requests:{" "}
                    {this.props.userRequest.friend_request.length - 1}
                  </Text>
                </TouchableOpacity>
                ...
                <TouchableOpacity
                  ...
                  onPress={() =>
                    this.setState({
                      removeFriendsOn: !this.state.removeFriendsOn,
                    })
                  }
                >
            ...
            >
              ...
              <FlatList
                ...
                renderItem={({ item }) => {
                  var gameName = item.in_game.slice(
                    0,
                    item.in_game.indexOf("-")
                  );
